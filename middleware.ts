...
export async function middleware(req: NextRequest) {
  ...
  // 1) Cek NextAuth session (buat user yang login dari browser)
  const sessionToken = await getToken({
    req,
    secret: NEXTAUTH_SECRET,
  });

  // 2) Cek Bearer token (buat Postman / service lain)
  ...
  // 3) Kalau Bearer token valid → izinkan (bypass role check)
  if (hasValidBearer) {
    return NextResponse.next();
  }

  // 4) Kalau tidak ada session → redirect ke login
  if (!sessionToken) {
  ...
  // 6) Route /work/** → semua user yang login boleh akses
